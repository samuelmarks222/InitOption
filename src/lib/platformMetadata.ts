...
const writeStorageValue = (key: string, value: string) => {
  try {
    ...
    // Some embedded browsers expose localStorage inconsistently.
  }
};
...
const removeStorageValue = (key: string) => {
  try {
    ...
    // Ignore storage write failures in constrained browser environments.
  }
};
...
export const readStoredPlatformName = () => {
  ...
  return (
    readStorageValue(SITE_PLATFORM_NAME_STORAGE_KEY) ||
...
export const readStoredLogoUrl = () => {
  ...
  const storedLogoUrl = readStorageValue(SITE_LOGO_STORAGE_KEY);
...
export const readStoredSupportEmail = () => {
  ...
  return (
    readStorageValue(SITE_SUPPORT_EMAIL_STORAGE_KEY) ||
...
export const applyPlatformSettingsToDocument = (
  ...
  if (typeof window !== "undefined") {
    writeStorageValue(SITE_PLATFORM_NAME_STORAGE_KEY, settings.platform_name);
    writeStorageValue(SITE_SUPPORT_EMAIL_STORAGE_KEY, settings.support_email);
  }
  ...
  if (resolvedLogoUrl) {
    ...
    if (typeof window !== "undefined") {
      writeStorageValue(SITE_LOGO_STORAGE_KEY, resolvedLogoUrl);
    }
  } else {
  ...
  upsertManagedLinkElement('link[rel="alternate"][hreflang="en"]', {
    rel: "alternate",
    hreflang: "en",
    href: resolvedSeo.canonicalUrl,
  });
  upsertManagedLinkElement('link[rel="alternate"][hreflang="x-default"]', {
    rel: "alternate",
    hreflang: "x-default",
    href: resolvedSeo.canonicalUrl,
  });
  upsertManagedLinkElement('link[rel="sitemap"]', {
    rel: "sitemap",
  ...
  upsertMetaElement("name", "application-name", settings.platform_name);
  upsertMetaElement("name", "language", "en");
  ...
  upsertMetaElement("property", "og:locale", "en_US");
