        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
            {CO_ADMIN_ROLE_OPTIONS.map((option) => (
              <div key={option.value} className="rounded-2xl border border-white/5 bg-[#11161d] p-4 shadow-lg">
                <div className="text-[11px] uppercase tracking-wider text-gray-500">{option.label}</div>
          ...
          <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
              ...
              <div className="border-b border-white/5 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">Pick a platform user to promote into a focused co-admin role.</p>
                  </div>
                </div>
                <div className="relative mt-4">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-white/10 bg-[#0b0e14] py-3 pl-10 pr-4 text-sm text-white outline-none transition-colors placeholder:text-gray-500 focus:border-blue-500"
                  />
                </div>
              </div>
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 text-xs text-gray-500">@{getUserHandle(profile)}</div>
                        </div>
                        <div className="rounded-full border border-white/10 bg-white/5 px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-gray-300">
                          {currentRole ? getRoleLabel(currentRole) : "No staff role"}
                        </div>
                      </div>
                      <div className="mt-3 text-[11px] text-gray-500">Joined {formatDate(profile.created_at)}</div>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="space-y-6">
              <div className="rounded-2xl border border-white/5 bg-[#11161d] p-5 shadow-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">Choose the exact responsibility this account should handle.</p>
                  </div>
                </div>

                {selectedUser ? (
                  <div className="mt-5 space-y-5">
                    <div className="rounded-2xl border border-white/5 bg-[#0b0e14] p-4">
                      <div className="text-[11px] uppercase tracking-wider text-gray-500">Selected User</div>
                      <div className="mt-2 text-lg font-bold text-white">{getUserName(selectedUser)}</div>
                      <div className="mt-1 text-sm text-gray-400">@{getUserHandle(selectedUser)}</div>
                      <div className="mt-3 inline-flex rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-gray-300">
                        ...
                        <button
                          ...
                          className={`rounded-2xl border p-4 text-left transition-colors ${
                            selectedRole === option.value
                              ...
                              : "border-white/5 bg-[#0b0e14] hover:border-white/10 hover:bg-white/[0.03]"
                          }`}
                        >
                          <div className="flex items-center justify-between gap-3">
                            <div className="text-sm font-bold text-white">{option.label}</div>
                            {selectedRole === option.value ? <ShieldCheck className="h-4 w-4 text-blue-300" /> : null}
                          </div>
                          <p className="mt-2 text-xs leading-6 text-gray-400">{option.description}</p>
                        </button>
                      ))}
                    </div>
                    ...
                    <button
                      ...
                      disabled={assigning}
                      className="inline-flex items-center justify-center rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                  ...
                  <div className="mt-5 rounded-2xl border border-dashed border-white/10 bg-[#0b0e14] p-8 text-center text-sm text-gray-400">
              ...
              <div className="rounded-2xl border border-white/5 bg-[#11161d] shadow-lg">
                <div className="border-b border-white/5 p-4">
                  ...
                  <p className="mt-1 text-sm text-gray-400">Review every elevated account and revoke access when it is no longer needed.</p>
                </div>
                ...
                {staffUsers.length === 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[760px] text-left text-sm text-gray-300">
                      <thead className="bg-[#0b0e14] text-xs uppercase text-gray-500">
                        <tr>
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <div className="text-xs text-gray-500">@{getUserHandle(staffUser)}</div>
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-4">
                              <span className="inline-flex rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-gray-200">
                                {getRoleLabel(staffUser.role)}
                              </span>
                            </td>
                            <td className="px-6 py-4 text-gray-400">{formatDate(staffUser.created_at)}</td>
