    ...
    <div className="space-y-6 animate-in fade-in duration-300 max-w-6xl">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Manage real-time announcements, welcome bonuses, deposit bonuses, and referral commissions.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-[1.1fr,0.9fr] gap-6">
        <div className="space-y-6">
          <section className="bg-[#11161d] border border-white/5 rounded-2xl p-6 shadow-lg space-y-4">
            <div className="flex items-center gap-3">
              <Megaphone className="w-5 h-5 text-fuchsia-400" />
              <div>
                <h3 className="text-lg font-bold text-white">Announcements</h3>
                <p className="text-sm text-gray-400">Send platform updates instantly to all users, tiers, or selected IDs.</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:border-blue-500 outline-none"
                />
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:border-blue-500 outline-none resize-none"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Optional link</label>
                <input
                  type="text"
                  value={form.linkUrl}
              ...
              {form.audienceMode === "tiers" && (
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">VIP tiers</label>
                  <input
                    type="text"
                    value={form.tiersCsv}
              ...
              {form.audienceMode === "users" && (
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">User IDs</label>
                  <textarea
                    rows={3}
            ...
            <div className="pt-2 flex justify-end">
              <button
                ...
                disabled={sending}
                className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg text-sm font-bold transition-colors shadow-lg shadow-blue-500/20 disabled:opacity-60"
              >
                <Send size={16} />
          ...
          <section className="bg-[#11161d] border border-white/5 rounded-2xl p-6 shadow-lg space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">These settings apply to new events only and create notifications in real time.</p>
              </div>
            </div>

            {loadingBonuses ? (
              <div className="text-sm text-gray-400">Loading bonus settings...</div>
            ) : bonusSettings ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label className="flex items-center justify-between rounded-xl border border-white/5 bg-[#0b0e14] px-4 py-3">
                  <span className="text-sm text-gray-300">Enable welcome bonus</span>
                  <input type="checkbox" checked={bonusSettings.welcome_bonus_enabled} onChange={(e) => setBonusSettings((current) => current ? { ...current, welcome_bonus_enabled: e.target.checked } : current)} className="accent-blue-500" />
                </label>

                <label className="flex items-center justify-between rounded-xl border border-white/5 bg-[#0b0e14] px-4 py-3">
                  <span className="text-sm text-gray-300">Enable deposit bonus</span>
                  <input type="checkbox" checked={bonusSettings.deposit_bonus_enabled} onChange={(e) => setBonusSettings((current) => current ? { ...current, deposit_bonus_enabled: e.target.checked } : current)} className="accent-blue-500" />
                </label>

                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Welcome bonus amount</label>
                  <input type="number" value={bonusSettings.welcome_bonus_amount} onChange={(e) => setBonusSettings((current) => current ? { ...current, welcome_bonus_amount: Number(e.target.value) } : current)} className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white font-mono focus:border-blue-500 outline-none" />
                </div>

                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Welcome trigger</label>
                  <select value="first_deposit" disabled className="w-full cursor-not-allowed bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white/80 focus:border-blue-500 outline-none disabled:opacity-100">
                    <option value="first_deposit">On first deposit</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Deposit bonus %</label>
                  <input type="number" value={bonusSettings.deposit_bonus_percent} onChange={(e) => setBonusSettings((current) => current ? { ...current, deposit_bonus_percent: Number(e.target.value) } : current)} className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white font-mono focus:border-blue-500 outline-none" />
                </div>

                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Minimum deposit</label>
                  <input type="number" value={bonusSettings.deposit_bonus_min} onChange={(e) => setBonusSettings((current) => current ? { ...current, deposit_bonus_min: Number(e.target.value) } : current)} className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white font-mono focus:border-blue-500 outline-none" />
                </div>

                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Maximum bonus</label>
                  <input type="number" value={bonusSettings.deposit_bonus_max} onChange={(e) => setBonusSettings((current) => current ? { ...current, deposit_bonus_max: Number(e.target.value) } : current)} className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white font-mono focus:border-blue-500 outline-none" />
                </div>

                <label className="flex items-center justify-between rounded-xl border border-white/5 bg-[#0b0e14] px-4 py-3 md:col-span-2">
                  <span className="text-sm text-gray-300">Enable referral commissions</span>
                  <input type="checkbox" checked={bonusSettings.referral_commission_enabled} onChange={(e) => setBonusSettings((current) => current ? { ...current, referral_commission_enabled: e.target.checked } : current)} className="accent-blue-500" />
                </label>

                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Referral commission %</label>
                  <input type="number" value={bonusSettings.referral_commission_percent} onChange={(e) => setBonusSettings((current) => current ? { ...current, referral_commission_percent: Number(e.target.value) } : current)} className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white font-mono focus:border-blue-500 outline-none" />
                </div>

                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Commission basis</label>
                  <select value={bonusSettings.referral_commission_type} onChange={(e) => setBonusSettings((current) => current ? { ...current, referral_commission_type: e.target.value } : current)} className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:border-blue-500 outline-none">
                ...
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Payout timing</label>
                  <select value={bonusSettings.referral_commission_payout_timing} onChange={(e) => setBonusSettings((current) => current ? { ...current, referral_commission_payout_timing: e.target.value } : current)} className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:border-blue-500 outline-none">
              ...
              <div className="rounded-xl border border-red-500/20 bg-red-500/5 px-4 py-4 text-sm text-red-200 space-y-3">
                ...
                {hasAdminDbRole === false && user?.id ? (
                  <div className="rounded-lg border border-white/10 bg-black/20 p-3 text-xs text-red-100/85 space-y-2">
              ...
              <div className="rounded-xl border border-white/5 bg-[#0b0e14] px-4 py-4 text-sm text-gray-400">
        ...
        <section className="bg-[#11161d] border border-white/5 rounded-2xl overflow-hidden shadow-lg flex flex-col min-h-[520px]">
          <div className="p-4 border-b border-white/5 bg-[#1A1F26]">
            <h3 className="text-white font-bold">Recent Broadcasts</h3>
            <p className="mt-1 text-xs text-gray-500">Latest sent and scheduled announcements.</p>
          </div>
          ...
          <div className="p-4 space-y-4 flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="text-sm text-gray-400">No announcements have been created yet.</div>
            ) : (
              announcements.map((announcement) => (
                <div key={announcement.id} className="bg-[#0b0e14] border border-white/5 rounded-xl p-4 relative">
                  <div className="absolute top-4 right-4 text-gray-500 text-xs flex items-center gap-1">
                  ...
                  <p className="text-xs text-gray-400 leading-relaxed">{announcement.message}</p>
