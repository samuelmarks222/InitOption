    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Create and monitor promotional marketing codes (LIVE DB).</p>
        </div>
        ...
        <div className="bg-[#11161d] border border-white/5 rounded-2xl p-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 items-end">
            <div>
              ...
              <input type="text" value={newPromo.code} onChange={e => setNewPromo({...newPromo, code: e.target.value})} placeholder="WELCOME100" className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:border-blue-500 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Type</label>
              <select value={newPromo.type} onChange={e => setNewPromo({...newPromo, type: e.target.value})} className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:border-blue-500 outline-none">
                <option value="Percentage">Percentage</option>
                <option value="Fixed Bonus">Fixed Bonus</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Value</label>
              <input type="text" value={newPromo.reward_value} onChange={e => setNewPromo({...newPromo, reward_value: e.target.value})} placeholder="e.g. 50% or $100" className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:border-blue-500 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Max Usages</label>
              <input type="number" value={newPromo.max_usages} onChange={e => setNewPromo({...newPromo, max_usages: Number(e.target.value)})} className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:border-blue-500 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Expiry Date</label>
              <input type="date" value={newPromo.expiry_date} onChange={e => setNewPromo({...newPromo, expiry_date: e.target.value})} className="w-full bg-[#0b0e14] border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:border-blue-500 outline-none" />
            </div>
      ...
      <div className="bg-[#11161d] border border-white/5 rounded-2xl overflow-hidden shadow-lg">
        <div className="p-4 border-b border-white/5 flex justify-between items-center bg-[#1A1F26]">
          <div className="flex gap-2 relative w-full max-w-sm">
             ...
             <input type="text" placeholder="Search code..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-[#0b0e14] border border-white/10 rounded-lg pl-9 pr-4 py-2 text-sm text-white focus:border-blue-500 outline-none" />
          </div>
        </div>
          ...
          <table className="w-full min-w-[760px] text-left text-sm text-gray-300">
            ...
            <tbody className="divide-y divide-white/5">
               {loading ? (
                 ...
                 <tr><td colSpan={6} className="text-center py-8 text-gray-500">No promo codes found in database.</td></tr>
               ) : filteredPromos.map((promo) => (
                <tr key={promo.id} className="hover:bg-white/[0.02]">
                  <td className="px-6 py-4 font-mono font-bold text-white text-lg tracking-wider bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent w-fit inline-block">
                    {promo.code}
                  </td>
                  <td className="px-6 py-4">
                    <div className="font-bold text-white">{promo.reward_value}</div>
                    <div className="text-[10px] uppercase tracking-wider text-gray-500 mt-0.5">{promo.type}</div>
                  </td>
                  <td className="px-6 py-4">
                    ...
                    <div className="w-24 h-1 bg-white/10 rounded-full mt-1.5 overflow-hidden">
                      <div className="h-full bg-blue-500" style={{ width: `${Math.min((promo.usages / promo.max_usages) * 100, 100)}%` }}></div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                     <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider w-fit ${
                        promo.status === "active" ? "bg-green-500/10 text-green-400" : "bg-gray-500/10 text-gray-500"
                      }`}>
                        {promo.status}
                      </span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => handleDelete(promo.id)} className="p-1.5 text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded transition-colors" title="Delete Promo">
                      <Trash2 size={16} />
                    </button>
                  </td>
                </tr>
