...
const HIDDEN_NOTIFICATION_TYPES = new Set(["trade_result"]);
const PROFILE_REFRESH_NOTIFICATION_TYPES = new Set([
  "welcome_bonus",
  "deposit_bonus",
  "referral_commission",
  "deposit_approved",
...
const sortNotifications = (items: AppNotification[]) =>
  [...items]
    .filter((item) => !HIDDEN_NOTIFICATION_TYPES.has(item.type))
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (HIDDEN_NOTIFICATION_TYPES.has(nextNotification.type)) {
            return;
          }
          ...
          if (initializedRef.current) {
            ...
            if (PROFILE_REFRESH_NOTIFICATION_TYPES.has(nextNotification.type)) {
              if (
                nextNotification.type === "welcome_bonus" ||
                nextNotification.type === "deposit_bonus" ||
                nextNotification.type === "referral_commission"
              ) {
