...
const Features = ({ content }: FeaturesProps) => {
  ...
  return (
    <section id="features" className="relative overflow-hidden bg-[#fffaf4] py-16 sm:py-20 lg:py-24">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_12%_10%,rgba(255,126,58,0.12),transparent_18%),radial-gradient(circle_at_88%_18%,rgba(76,109,255,0.08),transparent_20%),linear-gradient(180deg,#fffaf4_0%,#f1e8dc_100%)]" />

      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
          <div className="xl:sticky xl:top-32 xl:self-start">
            <div className="landing-neo-chip inline-flex items-center gap-2 rounded-full px-4 py-2 font-landing-copy text-[11px] font-semibold uppercase tracking-[0.22em] text-[#6d5d52]">
              <Sparkles className="h-3.5 w-3.5 text-[#ff6a2b]" />
              Platform direction
            </div>

            <h2 className="font-landing-display mt-5 max-w-[10ch] text-[clamp(2.2rem,5vw,4.5rem)] font-semibold leading-[0.96] tracking-[-0.06em] text-[#151117]">
              A homepage that looks like a real brand, not a recycled template.
            </h2>

            <p className="font-landing-copy mt-6 max-w-xl text-[15px] leading-8 text-[#62564e] sm:text-[17px]">
            ...
            <div className="landing-neo-card-dark mt-8 rounded-[34px] p-6 text-white">
              ...
              <Link
                to="/register"
                className="mt-8 inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 font-landing-copy text-sm font-semibold uppercase tracking-[0.16em] text-[#17131a]"
              >
                Open account
            ...
            {content.features.cards.map((card, index) => (
              <article
                ...
                className={`rounded-[32px] p-6 sm:p-7 ${
                  index === 1 ? "landing-neo-card-dark text-white" : "landing-neo-card text-[#17131a]"
                }`}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className={`font-landing-copy text-[10px] font-semibold uppercase tracking-[0.2em] ${index === 1 ? "text-white/56" : "text-[#85766a]"}`}>
                  ...
                  <div
                    className={`rounded-full px-3 py-1.5 font-landing-copy text-[10px] font-semibold uppercase tracking-[0.16em] ${
                      index === 1
                        ...
                        : "border border-[#1a1820]/8 bg-white/70 text-[#4f463e]"
                    }`}
                  >
                ...
                <p className={`font-landing-copy mt-5 text-[15px] leading-8 ${index === 1 ? "text-white/72" : "text-[#60554c]"}`}>
                  {card.text}
                </p>
              </article>
            ))}
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
          <article className="landing-neo-card overflow-hidden rounded-[36px] p-6 sm:p-8">
            <div className="flex flex-col gap-4 border-b border-[#18131a]/8 pb-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <div className="font-landing-copy text-[10px] font-semibold uppercase tracking-[0.22em] text-[#87776b]">
                  New-user path
                </div>
                <h3 className="font-landing-display mt-3 text-[clamp(2rem,4vw,3.35rem)] font-semibold leading-[0.98] tracking-[-0.06em] text-[#17131a]">
                  {content.steps.title}
                </h3>
              </div>
              <p className="font-landing-copy max-w-2xl text-[15px] leading-8 text-[#60554c]">
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              {content.steps.items.map((item, index) => (
                <article
                  ...
                  className="rounded-[28px] border border-[#18131a]/8 bg-white/82 p-5 shadow-[0_18px_44px_rgba(18,16,22,0.05)]"
                >
                  <div className="font-landing-display text-[34px] font-semibold tracking-[-0.06em] text-[#ff6a2b]">
                    {RAIL_NUMBERS[index] ?? `0${index + 1}`}
                  </div>
                  <h4 className="font-landing-display mt-6 text-[24px] font-semibold leading-[1.06] tracking-[-0.05em] text-[#17131a]">
                    {item.title}
                  </h4>
                  <p className="font-landing-copy mt-4 text-[15px] leading-8 text-[#62574f]">
                    {item.text}
                  </p>
                  <div className="font-landing-copy mt-5 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#8a7b6f]">
                    {item.cta}
                  </div>
                </article>
              ))}
