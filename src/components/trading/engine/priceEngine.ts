...
export class OTCPriceEngine {
  ...
  generateHistory(tfConfig: TimeframeConfig, nowSec = Date.now() / 1000): OHLCCandle[] {
    let candles: OHLCCandle[];

    if (tfConfig.seconds < TIMEFRAMES["1m"].seconds) {
      candles = this.aggregateFromBase(tfConfig, TIMEFRAMES["1s"], nowSec);
    } else if (tfConfig.seconds <= TIMEFRAMES["1h"].seconds) {
      candles = this.aggregateFromBase(tfConfig, TIMEFRAMES["1m"], nowSec);
    } else if (tfConfig.seconds <= TIMEFRAMES["1D"].seconds) {
      candles = this.aggregateFromBase(tfConfig, TIMEFRAMES["1h"], nowSec);
    } else {
      candles = this.getClosedHistory(tfConfig, tfConfig.historical, nowSec);
    }
