...
export const DrawingProperties = ({ onClose }: Props) => {
  ...
  const setColor = (c: string) => updateDrawing(d.id, {
    style: {
      ...d.style,
      color: c,
      fillColor: getDrawingToolFillColor(d.tool, c),
    },
  });
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs font-semibold text-white">{getDrawingToolLabel(d.tool)}</span>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <input type="range" min={1} max={5} value={d.style.lineWidth}
          ...
          className="flex-1 h-1 accent-[#0fa053]" />
      </div>
      ...
      <div className="flex gap-1 mb-3">
        {(["solid", "dashed", "dotted"] as const).map(s => (
          <button key={s} onClick={() => setStyle(s)}
            className={`flex-1 py-1 text-[10px] rounded transition-colors ${d.style.lineStyle === s ? "bg-[#0fa053]/30 text-[#8be0af]" : "bg-white/5 text-gray-400 hover:bg-white/10"}`}>
            {s}
          </button>
        ))}
      </div>
