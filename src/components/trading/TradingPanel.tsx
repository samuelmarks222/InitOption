...
const TradingPanel = ({ asset, balance, onTrade }: TradingPanelProps) => {
  ...
  const handleTrade = async (direction: "higher" | "lower") => {
    if (!onTrade) return;
    setLoading(true);
    await onTrade(asset.symbol, direction, investAmount, asset.price, expirationValue, profit / 100);
    setLoading(false);
  };
    ...
    <aside className="w-64 bg-card border-l border-border flex flex-col">
      ...
      <div className="p-4 border-b border-border">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => adjustAmount(-100)} className="w-6 h-6 rounded bg-secondary flex items-center justify-center text-muted-foreground hover:text-foreground">
      ...
      <div className="p-4 border-b border-border">
        ...
        <div className="relative">
          ...
          >
            <span className="text-lg font-semibold text-foreground">{formatExpiry(expirationValue)}</span>
            <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform ${showExpiryDropdown ? "rotate-180" : ""}`} />
          </button>
          {showExpiryDropdown && (
            <>
              <div className="fixed inset-0 z-40" onClick={() => setShowExpiryDropdown(false)} />
              <div className="absolute left-0 right-0 top-full mt-1 bg-card border border-border rounded-lg shadow-xl z-50 max-h-[200px] overflow-y-auto">
                {expiryTimes.map((expiry) => (
                  <button
                    key={expiry.value}
                    onClick={() => { setExpirationValue(expiry.value); setShowExpiryDropdown(false); }}
                    className={`w-full px-3 py-2 text-left text-sm hover:bg-secondary transition-colors ${expirationValue === expiry.value ? "text-primary bg-secondary" : "text-foreground"}`}
                  >
                    {expiry.label}
                  </button>
                ))}
        ...
        <div className="flex gap-2 mt-2">
          {[30, 60, 120, 300].map((sec) => (
            <button
              ...
              className={`flex-1 py-1 text-xs rounded ${expirationValue === sec ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"}`}
            >
      ...
      <div className="p-4 border-b border-border">
        ...
        <div className="text-lg font-semibold text-trading-green">+$ {((investAmount * profit) / 100).toFixed(0)}</div>
      </div>

      {/* Trading Buttons */}
      <div className="p-4 flex-1 flex flex-col gap-3">
        <Button variant="higher" size="trading" onClick={() => handleTrade("higher")} disabled={loading} className="flex flex-col items-center justify-center">
          <TrendingUp className="w-8 h-8 mb-1" />
          <span className="text-lg">HIGHER</span>
        </Button>
        <Button variant="lower" size="trading" onClick={() => handleTrade("lower")} disabled={loading} className="flex flex-col items-center justify-center">
      ...
      <div className="p-3 border-t border-border text-center">
        <span className="text-xs text-muted-foreground">CURRENT TIME: {new Date().toLocaleTimeString()} (UTC+3)</span>
      </div>
    </aside>
  );
};
