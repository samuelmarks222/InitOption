...
import { Lock, Shield, Bell, Monitor, Smartphone, Globe, LogOut, Wallet } from "lucide-react";
import { useCurrency } from "@/contexts/CurrencyContext";
import { AccountCurrencyModal } from "./AccountCurrencyModal";

export const ProfileSettings = () => {
  const [activeSettingsTab, setActiveSettingsTab] = useState<"account" | "notifications" | "interface" | "security">("account");
        ...
        {activeSettingsTab === "notifications" && (
          <div className="space-y-6">
            <SettingsSection title="Email Alerts">
              <div className="space-y-4">
                <ToggleRow title="Deposits & Withdrawals" description="Get an email when a transaction is completed." enabled={true} />
                <ToggleRow title="Trade Execution" description="Get notified when a pending order is executed." enabled={false} />
                <ToggleRow title="Promotions & Bonuses" description="Receive updates about new deposit bonuses and offers." enabled={true} />
              </div>
            </SettingsSection>
            
            <SettingsSection title="Push Notifications">
              <div className="space-y-4">
                <ToggleRow title="Price Alerts" description="Notify me when saved assets drop or rise significantly." enabled={true} />
                <ToggleRow title="Margin Calls" description="Crucial alerts regarding your trading margin balance." enabled={true} />
              </div>
            </SettingsSection>
          </div>
        )}
...
const SettingsTab = ({ id, icon: Icon, label, active, onSelect }: any) => (
  <button
    onClick={() => onSelect(id)}
...
const ToggleRow = ({ title, description, enabled, disabled }: any) => (
...
const Toggle = ({ isEnabled }: { isEnabled: boolean }) => (
  <div className={`w-10 h-5 rounded-full p-0.5 transition-colors duration-200 ease-in-out cursor-pointer ${isEnabled ? "bg-[#0b65c2]" : "bg-gray-600"}`}>
    <div className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform duration-200 ease-in-out ${isEnabled ? "translate-x-5" : "translate-x-0"}`} />
  </div>
);
