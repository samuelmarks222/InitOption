...
export const GuidedTour = ({ enabled = true }: GuidedTourProps) => {
  ...
  const position = useMemo(() => {
    ...
    if (!isDesktopViewport) {
      const width = Math.min(window.innerWidth - MOBILE_VIEWPORT_PADDING * 2, 360);
      return {
        left: clamp((window.innerWidth - width) / 2, MOBILE_VIEWPORT_PADDING, window.innerWidth - width - MOBILE_VIEWPORT_PADDING),
        top: clamp(
  ...
  return (
    <div className="fixed inset-0 z-[140]" aria-live="polite">
      ...
      <div
        ref={cardRef}
        className={`fixed rounded-[22px] border border-white/8 bg-[#242a39] p-6 text-left shadow-[0_20px_60px_rgba(5,16,45,0.42)] ${
          isDesktopViewport ? "" : "backdrop-blur-xl"
        }`}
      ...
      >
        ...
        <p className={`mt-6 font-medium leading-[1.35] text-[#e6edf9] ${isDesktopViewport ? "max-w-[230px] text-[18px]" : "max-w-full text-[17px]"}`}>
          {currentStep.message}
        </p>

        <div className={`mt-7 ${isDesktopViewport ? "" : "flex items-center justify-between gap-3"}`}>
          {!isDesktopViewport && (
            <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#7f91b1]">
              Mobile guide
            </div>
          )}
          <button
