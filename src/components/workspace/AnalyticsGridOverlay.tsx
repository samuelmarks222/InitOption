      ...
      <div className="w-full md:w-[90%] h-full bg-[#0E1217] shadow-2xl flex flex-col border-l border-white/10">
        ...
        <div className="bg-[#14181F] px-4 py-3 border-b border-[#ffffff10] shrink-0">
          <div className="flex overflow-x-auto gap-3 scrollbar-hide">
            ...
            <KPICard label="Win Rate" value={`${tradeStats.winRate}%`} color={tradeStats.winRate >= 50 ? "text-[#00C076]" : "text-orange-400"} />
          ...
          {TABS.map(tab => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1 px-3 py-2 rounded-xl shrink-0 whitespace-nowrap transition-colors snap-start ${isActive ? "bg-blue-500/15 text-blue-400" : "text-gray-500"}`}
              >
                <tab.icon className="w-4 h-4" />
                <span className="text-[9px] font-bold text-center leading-tight max-w-[60px]">
                  {tab.label.split(" ").slice(0, 2).join(" ")}
                </span>
                {isActive && <div className="w-4 h-[2px] bg-blue-400 rounded-full" />}
              </button>
            );
          })}
        </div>
