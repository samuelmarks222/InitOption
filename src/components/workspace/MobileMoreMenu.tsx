...
import { X, BarChart2, TrendingUp, Trophy, ChevronRight, Wallet, ArrowDownLeft, List, History, LogOut, Globe, ChevronDown, Award } from "lucide-react";
import { useAuth } from "@/contexts/AuthContext";
import { DepositModal, WithdrawalModal, AccountType } from "@/components/trading/AccountModals";
...
const SECTIONS = [
  { id: "market",      icon: BarChart2,   label: "Market",      action: "market_overview" },
...
export const MobileMoreMenu = ({ onClose, onOpenOverlay }: MobileMoreMenuProps) => {
  ...
  const liveBalance = profile?.balance ?? 0;
  ...
  return (
    <div className="flex flex-col h-full bg-[#121f27]">
      ...
      {showWithdrawal && <WithdrawalModal balance={liveBalance} onClose={() => setShowWithdrawal(false)} />}
    </div>
  );
};
...
export const MobileLeaderboardOverlay = ({ onClose }: LeaderboardOverlayProps) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0d1117]">
      ...
      <div className="flex items-center border-b border-white/5 px-4 shrink-0 bg-[#111518]">
        {(["weekly","monthly","all"] as const).map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`capitalize text-[12px] font-bold py-3 mr-5 border-b-2 transition-colors ${tab === t ? "text-orange-400 border-orange-400" : "text-gray-500 border-transparent"}`}
          >
      ...
      {/* Your rank row */}
      <div className="px-4 py-3 border-b border-white/5 flex items-center gap-3 bg-orange-500/5 shrink-0">
        <span className="text-gray-500 text-[13px] w-6 text-center">—</span>
        <div className="w-8 h-8 rounded-full bg-orange-500/30 flex items-center justify-center text-[11px] font-bold text-orange-400 shrink-0">You</div>
        <span className="text-[13px] text-orange-400 font-semibold flex-1">Your Name</span>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center gap-4">
        ...
        <div>
          ...
          <p className="text-[13px] text-gray-500 leading-relaxed">
            ...
            <span className="text-orange-400 font-semibold">Become the first one!</span>
          </p>
        </div>
      </div>
    </div>
