...
const ARTICLES = [
  {
    ...
    category: "Account",
    categoryColor: "from-[#0fa053] to-[#1e2330]",
    categoryBg: "bg-[#0fa053]/10",
    categoryText: "text-[#0fa053]",
    icon: Shield,
    readTime: "4 min",
  ...
  },
  {
    ...
    icon: CreditCard,
    readTime: "4 min",
    content: `## Depositing Funds\n\nDeposits can follow either the automatic crypto confirmation flow or the finance review flow, depending on the payment method you choose.\n\n**Step 1 - Open the Deposit Form**\nClick the green + Deposit button in the top header, or go to Account > Deposit Funds.\n\n**Step 2 - Choose an Enabled Deposit Method**\n\n- Use one of the live payment methods shown in the deposit flow.\n- Supported crypto methods open a hosted Plisio checkout and can credit automatically after the required confirmations are detected.\n- Static/manual methods still stay pending until finance reviews them.\n- If no deposit method appears, ask an admin to enable a supported crypto method or manual funding source first.\n\n**Step 3 - Complete the Payment**\nEnter the amount, continue to the hosted Plisio invoice, and send the payment there. Automatic crypto methods update after the webhook or listener reports enough confirmations, while manual methods remain pending for finance approval.\n\n**Bonus Offers**\nPromo and welcome bonuses are applied when the deposit is credited.\n\n**Deposit Limits**\n- Each enabled payment method shows its own minimum and maximum before checkout.\n- Crypto methods use the hosted invoice and network rules shown during the deposit flow.`,
  },
];

const VIDEOS = [
  { id: "v1", title: "Getting Started: Your First Trade", duration: "6:24", category: "Basics", color: "from-[#0fa053] to-[#1e2330]", emoji: "📊" },
  { id: "v2", title: "How to Read Candlestick Charts", duration: "8:15", category: "Basics", color: "from-[#1e2330] to-[#1e2330]", emoji: "🕯️" },
...
const GUIDE_SECTIONS = [
  ...
  { id: "trading-basics", title: "Trading Basics", icon: TrendingUp, color: "text-[#0fa053]", articles: ["how-to-trade", "indicators-guide"] },
  ...
  { id: "platform-features", title: "Platform Features", icon: Target, color: "text-[#8fb0cf]", articles: ["tournaments-rules", "indicators-guide"] },
];
              ...
              {related.map(a => {
                ...
                return (
                  <button key={a.id} onClick={() => onOpen(a.id)} className="bg-[#1A1F26] border border-white/5 hover:border-white/20 rounded-xl p-5 text-left group transition-all hover:scale-[1.02]">
                    ...
                    <div className="text-[13px] font-bold text-white group-hover:text-[#0fa053] transition-colors">{a.title}</div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between px-4 md:px-8 border-b border-white/5 bg-[#111518] shrink-0 gap-2 md:gap-0 pt-3 md:pt-0">
        ...
        <div className="flex items-center justify-between md:w-auto w-full">
          <div className="flex items-center gap-3 py-2 md:py-4">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#0fa053] to-[#1e2330] flex items-center justify-center">
        ...
        <div className="flex items-center gap-1 overflow-x-auto scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0 snap-x snap-mandatory">
          {NAV.map(n => (
            ...
            >
              {n.label}
              {view === n.id && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-gradient-to-r from-[#0fa053] to-[#1e2330] rounded-t" />}
            </button>
          ))}
        </div>
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-[#0f172a] via-[#1e1b4b] to-[#0E1217] px-4 sm:px-6 md:px-10 pt-10 md:pt-14 pb-12 md:pb-16">
            ...
            <div className="relative z-10 max-w-[900px] mx-auto text-center">
              ...
              <h1 className="text-[28px] sm:text-[34px] md:text-[44px] font-bold text-white mb-3 leading-tight">
                Hello, how can we <span className="bg-gradient-to-r from-[#0fa053] to-[#1e2330] bg-clip-text text-transparent">help you?</span>
              </h1>
              ...
              <div className="relative max-w-[600px] mx-auto">
                ...
                <input
                  ...
                  className="w-full pl-12 md:pl-14 pr-4 md:pr-5 py-3.5 md:py-4 bg-white/5 border border-white/10 hover:border-white/20 focus:border-[#0fa053]/60 rounded-2xl text-white text-[14px] md:text-[15px] outline-none transition-all placeholder-gray-600 backdrop-blur"
                />
                {searchQuery && (
              ...
              <div>
                ...
                {searchResults.length === 0 ? (
                  <div className="bg-[#1A1F26] rounded-2xl border border-white/5 p-10 text-center">
                    ...
                    <p className="text-gray-400 text-[14px]">No results found. <button onClick={() => setView("chat")} className="text-[#0fa053] hover:text-[#8be0af] underline">Chat with support</button></p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {searchResults.map(a => {
                      const Icon = a.icon;
                      return (
                        <button key={a.id} onClick={() => openArticle(a.id)} className="w-full flex items-center gap-3 md:gap-4 p-4 md:p-5 bg-[#1A1F26] rounded-xl border border-white/5 hover:border-[#0fa053]/30 text-left group transition-all">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-[14px] font-bold text-white group-hover:text-[#0fa053] transition-colors break-words">{a.title}</div>
                          ...
                          <ChevronRight className="w-4 h-4 text-gray-600 group-hover:text-[#0fa053] transition-colors shrink-0" />
                        </button>
                      );
                    })}
                  </div>
            ...
            {!searchQuery && (
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
                  <button onClick={() => setView("guide")} className="group relative overflow-hidden bg-[#1A1F26] rounded-2xl border border-white/5 hover:border-[#0fa053]/30 p-5 md:p-7 text-left transition-all hover:scale-[1.02] min-w-0">
                    <div className="absolute inset-0 bg-gradient-to-br from-[#1e2330]/15 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#0fa053] to-[#1e2330] flex items-center justify-center mb-5 shadow-lg shadow-[#0fa053]/25 group-hover:scale-110 transition-transform">
                    ...
                    <div className="flex items-center gap-1.5 text-[12px] font-bold text-[#0fa053]">Explore Guide <ChevronRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" /></div>
                  </button>
                  <button onClick={() => setView("videos")} className="group relative overflow-hidden bg-[#1A1F26] rounded-2xl border border-white/5 hover:border-[#1e2330]/40 p-5 md:p-7 text-left transition-all hover:scale-[1.02] min-w-0">
                    <div className="absolute inset-0 bg-gradient-to-br from-[#1e2330]/15 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#1e2330] to-[#1e2330] flex items-center justify-center mb-5 shadow-lg shadow-[#1e2330]/25 group-hover:scale-110 transition-transform">
                    ...
                    <div className="flex items-center gap-1.5 text-[12px] font-bold text-[#8fb0cf]">Watch Videos <ChevronRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" /></div>
                  </button>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      <div className="w-1 h-6 rounded-full bg-gradient-to-b from-[#0fa053] to-[#1e2330]" />
                    ...
                    {ARTICLES.map(a => {
                      ...
                      return (
                        <button key={a.id} onClick={() => openArticle(a.id)} className="flex items-center gap-4 p-5 bg-[#1A1F26] rounded-xl border border-white/5 hover:border-white/15 text-left group transition-all hover:bg-[#1e2535]">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-[13px] font-bold text-white group-hover:text-[#0fa053] transition-colors truncate">{a.title}</div>
                          ...
                          <ChevronRight className="w-4 h-4 text-gray-700 group-hover:text-[#0fa053] transition-colors shrink-0" />
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#0f4c35] via-[#0d3326] to-[#0E1217] border border-green-500/20 p-5 md:p-8">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-5 md:gap-6">
                    <div className="w-16 h-16 rounded-2xl bg-green-500/20 border border-[#0fa053]/30 flex items-center justify-center shrink-0">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="max-w-[800px] mx-auto px-4 sm:px-6 md:px-8 py-8 md:py-10 space-y-3">
            {GUIDE_SECTIONS.map(section => {
              ...
              return (
                <div key={section.id} className={`rounded-2xl border overflow-hidden transition-all ${isOpen ? "border-[#0fa053]/25 bg-[#13192b]" : "border-white/5 bg-[#1A1F26]"}`}>
                  <button onClick={() => setExpandedGuide(isOpen ? null : section.id)} className="w-full flex items-center gap-4 p-5 text-left">
                    <div className={`w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center shrink-0 ${isOpen ? "bg-[#0fa053]/15" : ""}`}>
                      ...
                      {section.articles.map(id => {
                        ...
                        return (
                          <button key={id} onClick={() => openArticle(id)} className="w-full flex items-center gap-4 px-5 py-4 hover:bg-white/5 transition-colors group text-left">
                            ...
                            <ChevronRight className="w-4 h-4 text-gray-700 group-hover:text-[#0fa053] transition-colors" />
                          </button>
                        );
                      })}
                    </div>
            ...
            <div className="flex gap-2 mb-8 flex-wrap">
              {videoCategories.map(cat => (
                <button key={cat} onClick={() => setVideoFilter(cat)}
                  className={`px-4 py-1.5 rounded-full text-[12px] font-bold transition-all border ${videoFilter === cat ? "bg-[#1e2330]/35 text-[#8fb0cf] border-[#1e2330]/40 shadow-md shadow-[#1e2330]/10" : "text-gray-500 border-white/10 hover:border-white/20 hover:text-white"}`}>
                  {cat}
                </button>
              ))}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredVideos.map(v => (
                <div key={v.id} className="group bg-[#1A1F26] rounded-2xl border border-white/5 hover:border-white/20 overflow-hidden cursor-pointer transition-all hover:scale-[1.02] hover:shadow-xl">
                  ...
                  <div className="p-5">
                    <div className="text-[14px] font-bold text-white group-hover:text-[#8fb0cf] transition-colors leading-snug">{v.title}</div>
              ...
              <div key={i} className={`flex items-end gap-2.5 ${msg.from === "user" ? "justify-end" : "justify-start"}`}>
                ...
                <div className={`flex flex-col gap-1 max-w-[85%] sm:max-w-[68%] ${msg.from === "user" ? "items-end" : "items-start"}`}>
                  <div className={`px-4 py-3 rounded-2xl text-[13px] leading-relaxed ${
                    msg.from === "user"
                      ? "bg-gradient-to-br from-[#0fa053] to-[#1e2330] text-white rounded-br-sm shadow-lg shadow-[#0fa053]/20"
          ...
          <div className="px-4 md:px-6 py-4 border-t border-white/5 bg-[#111518] shrink-0">
            <div className="flex items-center gap-3 bg-[#1A1F26] border border-white/10 focus-within:border-[#0fa053]/50 rounded-2xl px-4 py-2.5 transition-colors">
              ...
              <button
                ...
                className="w-9 h-9 rounded-xl bg-[#0fa053] hover:bg-[#2a955e] disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center transition-all hover:scale-105 shrink-0"
              >
