...
const AssetManagement = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-[#1e2330] pb-6">
        <div>
      ...
      <div className="bg-[#1e2330] border border-[#1e2330] rounded-2xl overflow-hidden shadow-lg">
        <div className="p-4 border-b border-[#1e2330] flex flex-col md:flex-row justify-between items-center bg-[#1e2330] gap-4">
          
          <div className="flex space-x-1 bg-[#1c1f2d] p-1 rounded-lg w-full md:w-auto">
            {(["ALL", "CURRENCIES", "CRYPTO", "STOCKS", "COMMODITIES"] as TabType[]).map(tab => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-4 py-1.5 rounded-md text-xs font-bold transition-colors ${
                  activeTab === tab ? "bg-[#0fa053] text-white shadow-sm" : "text-slate-300 hover:text-white hover:bg-[#1e2330]"
                }`}
              >
                {tab}
              </button>
          ...
          <div className="flex gap-2 relative w-full md:w-80">
             ...
             <input 
               ...
               className="w-full bg-[#1c1f2d] border border-[#1e2330] rounded-lg pl-9 pr-4 py-2 text-sm text-white focus:border-[#0fa053] outline-none transition-colors" 
             />
          </div>
        </div>
        ...
        <div className="overflow-x-auto max-h-[700px] overflow-y-auto">
          <table className="w-full min-w-[760px] text-left text-sm text-slate-200">
            <thead className="sticky top-0 text-xs uppercase bg-[#1e2330] text-slate-300 border-b border-[#1e2330] z-10">
              <tr>
              ...
              {loading ? (
                ...
                return (
                  <tr key={ast.symbol} className={`transition-colors ${isActive ? "bg-[#0b65c2]/5" : "hover:bg-white/[0.02]"}`}>
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-1.5 rounded bg-[#1e2330] text-slate-200 text-[10px] font-bold tracking-wider">{ast.category}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="h-10 w-20 rounded-lg border border-[#1e2330] bg-[#1c1f2d] px-3 text-sm font-semibold text-white outline-none transition-colors focus:border-[#0fa053]"
                        />
                        <button
                          type="button"
                          onClick={() => handleSavePayout(ast, dbMatch)}
                          disabled={!isAdded || !payoutChanged || savingPayoutSymbol === ast.symbol}
                          className="rounded-lg border border-[#1e2330] px-3 py-2 text-[11px] font-bold text-white transition-colors hover:bg-[#1e2330] disabled:cursor-not-allowed disabled:opacity-40"
                        >
                    ...
                    <td className="px-6 py-4 text-right">
                       <button 
                         ...
                         className={`px-4 py-2 rounded-lg text-xs font-bold transition-all shadow-sm flex items-center gap-2 ml-auto ${
                           isActive 
                             ? "bg-[#1e2330] text-slate-300 hover:bg-red-500/20 hover:text-red-400" 
