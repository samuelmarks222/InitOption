...
type EmbedSpec = { alias: string; table: string; cols: string[] };

const parseColumns = (
  raw: string | undefined,
): { columns: string[]; embeds: EmbedSpec[] } | null => {
  if (!raw || raw.trim().length === 0) return null;
  if (raw.trim() === "*") return { columns: ["*"], embeds: [] };
  ...
  for (const part of raw.split(",")) {
    ...
    // PostgREST embedded select: alias:table(cols) or table(cols)
    ...
    if (embedMatch) {
      ...
      const inner = embedMatch[3].split(",").map((ic) => ic.trim()).filter(Boolean);
      embeds.push({ alias, table, cols: inner });
      continue;
    }

    if (!IS_IDENTIFIER.test(trimmed)) return null;
...
const buildSelectAndJoins = (
  parsed: { columns: string[]; embeds: EmbedSpec[] },
  table: string,
): { selectList: string[]; joins: string[] } => {
  const selectList: string[] = [];
  const joins: string[] = [];

  for (const col of parsed.columns) {
    selectList.push(col === "*" ? "*" : `"${col}"`);
  }

  for (const emb of parsed.embeds) {
    const fk = `${emb.alias}_id`;
    joins.push(
      `left join public."${emb.table}" on public."${emb.table}".id = public."${table}"."${fk}"`,
    );

    if (emb.cols.length === 1 && emb.cols[0] === "*") {
      selectList.push(`to_jsonb(public."${emb.table}") as "${emb.alias}"`);
    } else {
      const obj = emb.cols
        .map((c) => `'${c}', public."${emb.table}"."${c}"`)
        .join(", ");
      selectList.push(`json_build_object(${obj}) as "${emb.alias}"`);
    }
  }

  return { selectList, joins };
};
...
export async function handleDb(request: ApiRequest, response: ApiResponse): Promise<void> {
  ...
  try {
    ...
    const formatSqlParam = (v: unknown): unknown => {
    ...
    if (method === "POST") {
      ...
      const { rows } = await runScoped(mappedId, (client) =>
        client.query(
          ...
          entries.map(([, v]) => formatSqlParam(v)),
        ),
      );
      sendJson(response, 200, { data: rows });
      return;
    ...
    if (method === "PATCH") {
      ...
      const params: unknown[] = entries.map(([, v]) => formatSqlParam(v));
